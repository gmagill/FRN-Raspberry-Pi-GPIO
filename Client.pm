...
my $RXTweek = 5;
my $port;
my $Rogerfile = "/opt/LinFRN/sounds/Over01.wav";
BEGIN {
...
sub new {
    ...
    if ($HAS_AUDIO_GSM > 0.03) {
        ...
        warn "No suitable sound modules found. Program will operate in silent mode. Install either Audio::GSM or Win32::ACM.\n";
    }
	my %Config;
	Config::Simple->import_from('/var/www/frnclient.conf', \%Config);
    my $self = {
        _socket        => undef,
        _select        => new IO::Select,
        _gsm           => $gsm,
        _VX            => FRN_PROTO_VERSION,
        _ON            => sprintf("%s, %s", $Config{Callsign}, $Config{Name}),
        _EA            => $Config{Email},
        _PW            => $Config{Password},
        _NT            => $Config{Net},
        _NN            => $Config{Country} || 'N/A',
        _CT            => sprintf("%s - %s", $Config{City} || 'N/A', $Config{Locator} || 'N/A'),
        _BC            => $Config{Type} || FRN_TYPE_PC_ONLY,
        _BN            => undef,
        _BP            => undef,
        _SV            => undef,
        _DS            => $Config{Desc} || '',
		...
		_PTTUSECOM	   => $Config{Usecom},
		_PTTCOMLINE	   => $Config{Comline},
		_PTTCOMPATH	   => $Config{Compath},
		_PTTPOLARITY   => $Config{PTTHigh},
		_VOXTIMEOUT    => $Config{VoxTimeout},
    };
    bless($self, $class);
    return $self;
}
...
sub run {
        my $self = shift;
	my $polarity = 2;
	if (!$self->{_PTTCOMPATH} == 0) {
        $port = Device::SerialPort->new($self->{_PTTCOMPATH});
        $port->baudrate(19200);
        $port->databits(8);
        $port->parity("none");
        $port->stopbits(1);
        if ($self->{_PTTPOLARITY} == 0) {
            $polarity = 1;
		} else { $polarity = 0; }
        $port->dtr_active($polarity);
        $port->rts_active($polarity);
        print "Switching Relay $polarity\n";
        system ("/opt/LinFRN/bin/setPTT.sh $polarity");
	} else {
        print ("Not Using PTT Keyer\n");
	}
    $self->_init;
...
sub _parse {
    ...
    if ($self->{_state} == STATE_DISCONNECTED) {
		...
		if ($result{AL} eq FRN_RESULT_OK) {
            ...
            return;
        } elsif ($result{AL} eq FRN_RESULT_ADMIN) {
		    $self->{_state} = STATE_LOGIN_PHASE2;
			print "Gateway is Channel Admin\n";
            return;
	    } elsif ($result{AL} eq FRN_RESULT_OWNER) {
		    $self->{_state} = STATE_LOGIN_PHASE2;
			print "Gateway is Channel Owner\n";
            return;
            ...
            print "Server has rejected your login!";
            $self->{_state} = STATE_ABORT;
            return;
        } else {
...
sub _ping {
    my $self = shift;
	my $polarity = 2;
	printf("%s\r", (qw(L R M - 2 7))[$rotoPhase++]);
	local $| = 1;
	$rotoPhase = 0 if $rotoPhase > 5;
	if ($RXENDTime eq $LastRXTime) {
		if ($RXTweek eq 1) {
            if (!$self->{_PTTCOMPATH} == 0) {
                &{$self->{_callback}{onPLAYWav}}();
                if ($self->{_PTTPOLARITY} == 0) {
                    $polarity = 1;
                } else { $polarity = 0;}
                $port->dtr_active($polarity);
...
sub _readPTT {
	...
	if (-e '/opt/LinFRN/spool/TX.PTT') {
        ...
        if ($NoLoopPTT eq 1) {
            if (!$self->{_VOXTIMEOUT} == 0) {
                sleep($self->{_VOXTIMEOUT});
            }
